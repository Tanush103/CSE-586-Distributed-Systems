...
filtered_dict_comedy = [{k:v for (k,v) in i.items() if 'Comedy' in i['genre']} for i in filter_tamil]
  #new_dict.update(filtered_dict)
filter_comedy=[x for x in filtered_dict_comedy if x]


filtered_dict_drama = [{k:v for (k,v) in i.items() if 'Drama' in i['genre']} for i in filter_tamil]
  #new_dict.update(filtered_dict)
filter_drama=[x for x in filtered_dict_drama if x]


filtered_dict_crime = [{k:v for (k,v) in i.items() if 'Crime' in i['genre']} for i in filter_tamil]
  #new_dict.update(filtered_dict)
filter_crime=[x for x in filtered_dict_crime if x]


filtered_dict_action = [{k:v for (k,v) in i.items() if 'Action' in i['genre']} for i in filter_tamil]
  #new_dict.update(filtered_dict)
